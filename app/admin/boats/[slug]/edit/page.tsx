...
export default function EditBoatPage() {
  ...
  useEffect(() => {
    async function loadBoat() {
      try {
        const boatData = await getBoatBySlug(slug);
        if (!boatData) {
          setError('Bote no encontrado');
          setIsLoading(false);
          return;
        }

        setBoat(boatData);
        setExistingMainImage(boatData.mainImage);
        setExistingImages(boatData.images || []);

        // Cargar datos del formulario
        setFormData({
          title: boatData.name,
          max_capacity: boatData.capacity.toString(),
          price_per_hour: boatData.price.toString(),
          pricing_type: boatData.pricing?.type || 'fixed',
          fixed_prices: boatData.pricing && boatData.pricing.type === 'fixed'
            ? {
                '2H': boatData.pricing.prices['2H']?.toString() || '',
                '3H': boatData.pricing.prices['3H']?.toString() || '',
                '4H': boatData.pricing.prices['4H']?.toString() || '',
                '5H': boatData.pricing.prices['5H']?.toString() || '',
                '6H': boatData.pricing.prices['6H']?.toString() || '',
                '7H': boatData.pricing.prices['7H']?.toString() || '',
                '8H': boatData.pricing.prices['8H']?.toString() || '',
              }
            : {
                '2H': '', '3H': '', '4H': '', '5H': '', '6H': '', '7H': '', '8H': '',
              },
          weekday_price: boatData.pricing && boatData.pricing.type === 'hourly'
            ? boatData.pricing.weekday.toString()
            : '',
          weekend_price: boatData.pricing && boatData.pricing.type === 'hourly'
            ? boatData.pricing.weekend.toString()
            : '',
          minimum_hours: boatData.pricing && boatData.pricing.type === 'hourly'
            ? boatData.pricing.minimum_hours.toString()
            : '',
          catalog_link: boatData.catalogLink || '',
          category: boatData.category || '',
          collection: boatData.collection || '',
          length: boatData.length || '',
          description: boatData.description || '',
          features: boatData.features.join(', '),
        });

        setIsLoading(false);
      } catch (err: any) {
        setError(err.message || 'Error al cargar el bote');
        setIsLoading(false);
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updateResult = await updateBoat(boat.id, {
      ...
      if (!updateResult.success) {
        setError(updateResult.error || 'Error al actualizar el bote');
        setIsSubmitting(false);
        return;
      }
      ...
      setError(err.message || 'Error al actualizar el bote');
      setIsSubmitting(false);
    }
  };
      ...
      <main className="relative z-10 w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
        <div className="mb-6 sm:mb-8 text-center sm:text-left">
          ...
          <p className="text-gray-400 text-xs sm:text-sm">
            Modifica la información del bote {boat.name}
          </p>
        </div>
        ...
        {success && (
          <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-green-500/20 border border-green-500/50 rounded-lg text-green-300 text-sm">
            ...
            <p>El bote se ha actualizado correctamente. Redirigiendo al catálogo...</p>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="w-full space-y-4 sm:space-y-6">
            ...
            <div className="space-y-3 sm:space-y-4">
              {existingMainImage && (
                <div>
                  ...
                  <p className="text-xs text-gray-400 mb-3">Sube una nueva imagen para reemplazarla</p>
                </div>
              )}
              
              <div>
                <label htmlFor="main_image" className="block text-xs sm:text-sm font-medium text-gray-300 mb-1.5 sm:mb-2">
                  Nueva Imagen Principal <span className="text-[10px] sm:text-xs text-gray-500">(opcional, main.jpg)</span>
                </label>
                <input
                  type="file"
                  id="main_image"
              ...
              {existingImages.length > 0 && (
                <div>
                  ...
                  <p className="text-xs text-gray-400 mb-3">Sube nuevas imágenes para reemplazarlas</p>
                </div>
              )}

              <div>
                <label htmlFor="additional_images" className="block text-xs sm:text-sm font-medium text-gray-300 mb-1.5 sm:mb-2">
                  Nuevas Imágenes Adicionales <span className="text-[10px] sm:text-xs text-gray-500">(opcional, múltiples)</span>
                </label>
                <input
                  type="file"
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
            ...
            >
              {isSubmitting ? 'Guardando...' : 'Actualizar Bote'}
            </button>
            <button
              type="button"
              onClick={() => router.back()}
